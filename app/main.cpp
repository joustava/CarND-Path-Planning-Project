...
int main(int argc, char* argv[]) {
  ...
  string map_file_ = argv[1]; //"../data/highway_map.csv";
...
&map_waypoints_s,
&map_waypoints_dx,
&map_waypoints_dy, &lane, &dist_inc, &ref_vel]
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      auto sample = hasData(data);

      if (sample != "") {
        auto j = json::parse(sample);
        ...
        if (event == "telemetry") {
          ...
          // planner.updatePath(previous_path_x, previous_path_y, end_path_s, end_path_d);
          ...
          spline.set_points(ptsx, ptsy); // create the anchor points on spline.
